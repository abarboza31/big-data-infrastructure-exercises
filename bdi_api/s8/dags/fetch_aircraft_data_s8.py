...
def fetch_files_from_s3(**kwargs):
    """Fetch multiple files from S3 raw layer."""
    day = kwargs['ds']
    aircraft_data_all = []
    num_files_to_fetch = 10  # Fetch 10 files (adjust as needed)
    for i in range(num_files_to_fetch):
        minute = f"{i:06d}"  # e.g., 000000Z, 000001Z, etc.
        file_key = f"raw/day={day}/{minute}Z.json.gz"
        try:
...
def process_raw_to_prepared(**kwargs):
    """Transform fetched data and store in S3 prepared layer."""
    ti = kwargs['ti']
    day = kwargs['ds']
    aircraft_data_all = ti.xcom_pull(task_ids='fetch_files_from_s3')
...
def upload_to_postgresql(**kwargs):
...
with DAG(
    ...
    process_task = PythonOperator(
        task_id="process_raw_to_prepared",
        python_callable=process_raw_to_prepared,
        provide_context=True,
    )
